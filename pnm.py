...
def read(filename):
    """
    Reads in a PGM/PP file by the given name and returns its contents in a new numpy
    ...
    if os.path.isfile(filename):
        with open(filename, "rb") as f:
            ...
            if match is not None:
                ...
                numch = 3 if typestr == "P6" else 1
                shape = (height, width, numch) if typestr == "P6" else (height, width)
                if VERBOSE:
                ...
                raise IOError("File %s is not a valid PGM/PPM file."%(filename))
    else:
